...
#include "IMG_Spraygun.h"            // Imported image (as bytestream) from PL Image Inverter tool 
#include "IMG_Cool.h"            // Imported image (as bytestream) from PL Image Inverter tool 
#include "IMG_LegioV4.h"            // Imported image (as bytestream) from PL Image Inverter tool 
...
void setup() {  
    ...
    epd.showImage(IMG_Spraygun);     // Load image byte stream and trigger an update
    delay(5000);

    epd.clearScreen(EPD_BLACK);     // ClearScreen
    epd.showImage(IMG_Cool);     // Load image byte stream and trigger an update
    delay(5000);

    epd.clearScreen(EPD_BLACK);     // ClearScreen
    epd.showImage(IMG_LegioV4);     // Load image byte stream and trigger an update
    delay(5000);
}
